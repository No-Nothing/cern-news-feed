...
# Sicherheits-Check: Falls Module fehlen, fangen wir das ab
try:
    import requests
    from feedgen.feed import FeedGenerator
    from datetime import datetime, timezone
except ImportError as e:
    print(f"!!! FEHLER: Ein benötigtes Python-Paket fehlt: {e}")
    sys.exit(1)
...
def generate_cern_feed():
    ...
    print("Starte Abfrage der CERN-API...")
    try:
        response = requests.get(api_url, headers=headers, timeout=15)
        response.raise_for_status()
        data = response.json()
        print("CERN-API erfolgreich abgefragt.")
    ...
    try:
        ...
        print(f"!!! FEHLER beim Erstellen des XML-Feeds: {e}")
